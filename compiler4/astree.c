...
{  
   ...
   switch (node->type) {
    ...
    case AST_ARGUMENT:
       ...
       if (node->child[0]->type == AST_VARREF) fprintf(out,"\tmv\ta%d, t0\n", hval);
       hval++;
       break;
    case AST_ASSIGNMENT:
       if (node->varKind == V_GLARRAY) { //child[1]) {
          fprintf(out,"array var\n");
          fprintf(out,"%s--index--\n",levelPrefix(hval));
          genCodeFromASTree(node->child[1],hval+1,out);
       }
       genCodeFromASTree(node->child[0],hval,out);  // child 1 is right hand side
       fprintf(out,"\tsw\tt0, %s, t1\n", node->strval);
       break;
    case AST_WHILE:
       fprintf(out,"While loop\n");
       genCodeFromASTree(node->child[0],hval,out);  // child 0 is condition expr
       fprintf(out,"%s--body--\n",levelPrefix(hval));
